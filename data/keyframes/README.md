...
`http://192.168.31.3:8000`。它读取本目录的中文提示词，默认让 ComfyUI 的特殊 Seed 节点
以 `-1` 为值随机生成 **10 张候选图**，取回二段细化结果后保存为精确的 `1280×768` PNG。
...
使用 `--samples 4` 可改为 4 张候选，`--seed 42001` 可使用固定 seed；`--force` 才会
重新生成已有候选。确认某张候选后，手动将它复制为对应正式关键帧，例如：
