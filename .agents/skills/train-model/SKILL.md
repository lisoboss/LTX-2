...
These are procedure documents the orchestrator reads when entering each phase. They are not standalone skills — they're never invoked by Codex's skill-discovery system. The orchestrator opens them via the `Read` tool and follows the instructions inline.
